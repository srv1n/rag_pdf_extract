// Vendored from `adobe-cmap-parser` 0.4.1 (MIT licensed), by Jeff Muizelaar.
// https://github.com/jrmuizel/adobe-cmap-parser
//
// Vendored (instead of a `[patch.crates-io]`) because this crate is consumed
// downstream as a git dependency, and Cargo only honours `[patch.crates-io]`
// from the workspace root, so a patch here would be silently ignored by the
// consumer. Vendoring keeps the fix in this crate's own source.
//
// Fix applied: `as_code_range` computed `width` by dividing the length of an
// already-DECODED byte vector by 2 (`start_chars.len() as u32 / 2`), which is
// wrong -- `start_chars` holds one entry per input byte, not per encoded hex
// pair. This made every 1-byte codespace (`<00> <FF>`) compute width 0, and
// every 2-byte codespace compute width 1, so downstream code that matches on
// `range.width` never matched, silently dropping all decoded text. Changed to
// `start_chars.len() as u32`.
//
// Un-vendor by dropping this file, restoring `adobe-cmap-parser = "0.4.x"` in
// Cargo.toml (once/if a version with the fix ships upstream), and reverting
// the `mod cmap;` wiring in lib.rs.
...
fn as_code_range(start_chars: &Vec<u8>, end_chars: &Vec<u8>) -> CodeRange {
    ...
    // Fixed: this used to be `start_chars.len() as u32 / 2`. `start_chars` is
    // already-decoded bytes (one entry per input byte), not hex-digit pairs,
    // so dividing by 2 halved every width -- a 1-byte codespace (`<00> <FF>`)
    // came out as width 0 and a 2-byte codespace as width 1. Callers that
    // match on `range.width` in 1..=4 then never matched a 1-byte codespace
    // at all, so `next_char` returned `None` on the first byte and whole
    // strings decoded to nothing.
    let width = start_chars.len() as u32;
...
mod tests {
    ...
    #[test]
    fn it_works() {
        // Minimal inline CMap snippet instead of the upstream example file
        // (which isn't vendored) -- just exercises the parser end to end.
        let contents = b"1 begincodespacerange\n<00> <FF>\nendcodespacerange\n";
        do_parse(contents);
    }
}
