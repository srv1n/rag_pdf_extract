...
impl<'a> PdfImage<'a> {
    ...
    pub fn to_rgb_image(&self, transform: Option<&Transform>) -> Result<RgbImage, Box<dyn Error>> {
        debug!(
            "Converting PDF image - {}x{}, color_space={:?}, filters={:?}",
            ...
            .and_then(|o| o.as_dict().ok())
        {
            ...
            if matches!(predictor_val, Some(12)) {
                ...
                debug!(
                    "Applying PNG-Up predictor (Predictor=12) with row size {}",
                    row
                );
            ...
            .map(|s| s.as_str())
        {
            ...
            Some("CCITTFaxDecode") => {
                ...
                return self.construct_raw_image(&bytes, transform).or_else(|_| {
    ...
    fn construct_raw_image(
        ...
        match (self.color_space.as_deref(), components) {
            (Some("Indexed"), _) => {
                // Indexed color space uses a palette
                // For now, treat as grayscale or RGB depending on component count
                debug!("Converting Indexed color space image (treating as RGB fallback)");
                if components == 1 {
                    // Treat as grayscale
                        ...
                        for x in 0..self.width as u32 {
                            ...
                            if idx + 2 < bytes.len() {
                                img.put_pixel(
                                    x,
                                    y,
                                    image::Rgb([bytes[idx], bytes[idx + 1], bytes[idx + 2]]),
                                );
                            }
                        }
                    }
            ...
            (None, 3) => {
                // No color space specified but 3 components - assume RGB
                debug!("No color space specified with 3 components, assuming RGB");
                for y in 0..self.height as u32 {
                    for x in 0..self.width as u32 {
                        let idx = ((y * self.width as u32 + x) * 3) as usize;
                        if idx + 2 < bytes.len() {
            ...
            _ => {
                // Last resort fallback - try to interpret as grayscale
                warn!(
                    "Unsupported color space: {:?} with {} components, falling back to grayscale",
